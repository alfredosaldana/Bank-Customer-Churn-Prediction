...
📋 Project Overview
Customer churn — the phenomenon of customers discontinuing their relationship with a business — is one of the most costly problems in retail banking. Acquiring a new customer costs five to seven times more than retaining an existing one, making early identification of at-risk customers a top strategic priority.
...
├── data/
│   ├── raw/
│   │   └── Customer-Churn-Records.csv       # Original Kaggle data (10,000 × 18)
│   ├── processed/                            # Cleaned & engineered datasets
│   └── exports/
│       └── churn_tableau_export.csv          # For Tableau dashboard
│
├── notebooks/                                 # 7 sequential notebooks
│   ├── N1_data_upload.ipynb                  # Load data, first look, data dictionary
│   ├── N2_ExploratoryDataAnalysis.ipynb      # EDA: distributions, correlations, leakage
│   ├── N3_data_cleaning.ipynb                # Drop identifiers, handle leakage, outliers
│   ├── N4_data_engineering.ipynb             # Split → encode → scale (CustomScaler)
│   ├── N5_Model_Train_Test_Selection.ipynb   # SMOTE → train 6 models → compare metrics
│   ├── N6_Model_Saving.ipynb                  # Retrain on full data, save artifacts
│   └── N7_BankChurn_Module.ipynb              # Demo the production inference module
│
├── artifacts/                                 # Generated by running notebooks
│   ├── model_file.pkl                         # Trained best model (Random Forest)
...
RowNumber	Integer	Sequential row index	❌ Dropped
CustomerId	Integer	Unique customer identifier	❌ Dropped
Surname	String	Customer last name	❌ Dropped
CreditScore	Integer	Credit score (300–850). Higher = better creditworthiness.	✅ Yes
Geography	String	Country: France, Germany, or Spain	✅ Encoded
Gender	String	Male or Female	✅ Encoded
Age	Integer	Customer age in years	✅ Yes
Tenure	Integer	Years as a bank customer (0–10)	✅ Yes
Balance	Float	Current account balance in Euros	✅ Yes
NumOfProducts	Integer	Number of bank products held (1–4)	✅ Yes
HasCrCard	Binary (0/1)	Has a credit card? 1 = Yes	✅ Yes
IsActiveMember	Binary (0/1)	Considered an active member? 1 = Yes	✅ Yes
EstimatedSalary	Float	Estimated annual salary in Euros	✅ Yes
Exited	Binary	TARGET: 1 = left the bank	🎯 Target
Complain	Binary	Filed a complaint? — ⚠️ DROPPED (leakage)	❌ Dropped
Satisfaction Score	Integer	Customer satisfaction rating (1–5)	✅ Yes
Card Type	String	DIAMOND / GOLD / SILVER / PLATINUM	✅ Encoded
Point Earned	Integer	Reward points accumulated	✅ Yes
Critical Note: Complain has a 0.99 Pearson correlation with Exited — they are 99.7% identical. Including it would constitute target leakage: the model would learn a near-perfect shortcut that only exists in historical data, not in real-world deployment.

🔬 Methodology: The Complete Pipeline
The Problem with Imbalanced Data
text
Stayed (0):  7,963 customers  ████████████████████████  79.6%
Exited (1):  2,037 customers  █████                      20.4%
A model predicting "stayed" for everyone would be 79.6% accurate — but useless. This project addresses imbalance using SMOTE (Synthetic Minority Oversampling Technique).
...
# After:  {0: 6370, 1: 6370}  ← balanced!
🧪 Pipeline Steps in Detail
Step 1 — N1: Data Upload & First Look
python
...
Step 2 — N2: Exploratory Data Analysis
...
Is there data leakage? (Yes — Complain is 99% correlated with Exited)
...
class CustomScaler:
    ...
    def transform(self, X):
        ...
        return X_copy
Step 5 — N5: Model Training & Selection
...
Step 7 — N7: Production Module Demo
python
...
Finding 1 — Geography matters significantly
...
Finding 2 — Age is the strongest individual predictor
Customers aged 40–60 are substantially more likely to churn. This may indicate a "switching window" where mid-career customers re-evaluate financial relationships as their wealth grows.

Finding 3 — Number of products is a double-edged sword

Customers with 2 products: lowest churn rate

Customers with 3–4 products: highest churn rate (counterintuitive)
This suggests forced bundling may cause dissatisfaction.

Finding 4 — Account balance and churn have a bimodal relationship
Both zero-balance customers (dormant/disengaged) and very high-balance customers (shopping competitors) show elevated churn.

Finding 5 — Active membership is strongly protective
...
🐍 Using the CustomerChurn Module
...
📊 Tableau Dashboard: "Bank Churn Command Center"
A business-facing interactive dashboard for exploring churn patterns without code.

Worksheet Specifications
Sheet 1 — Churn Rate by Geography (Bar/Map)

Shows Germany at ~32% vs France/Spain at ~16%

Sheet 2 — Age Distribution: Churners vs. Stayers (Dual histogram)

Visualizes the 40-60 age risk window

Sheet 3 — Churn Rate by Number of Products (Bar chart)

Reference line at overall average (20.4%)

Highlights 3-4 product anomaly

Sheet 4 — Feature Importance (Horizontal bar)

Top 15 features from Random Forest

Sheet 5 — Churn Probability Distribution (Histogram)
...
🎯 Why NOT Accuracy? — Evaluation Metrics Explained
Metric	Formula	Why it matters here
Precision	TP / (TP + FP)	Of all predicted churners, how many actually churned?
Recall	TP / (TP + FN)	Of all actual churners, how many did we catch? ← most important
F1-Score	Harmonic mean of P & R	Balance between precision and recall
ROC-AUC	Area under ROC curve	Overall separability — model-threshold independent
...
# 4. Run notebooks in order: N1 → N2 → N3 → N4 → N5 → N6 → N7
...
Educational dataset: This Kaggle dataset is simulated — findings may not generalize to real bank data without revalidation.
...
Binary features not scaled: HasCrCard and IsActiveMember remain 0/1 — intentional, but must be handled consistently.

🚀 Next Steps & Future Work
SHAP explainability: Add SHAP values to explain individual predictions — critical for regulatory and stakeholder acceptance
